...
class _MembershipCarouselManagerState
  ...
  String? _selectedDuration = 'weekly'; // Default to weekly
  bool _isActive = true;
  ...
  Future<void> _loadDefaultImage() async {
    ...
    if (_selectedImage == null) {
      final file = await AssetToFileHelper.assetToFile(
        ...
        fileName: 'default_thumbnail_package.webp',
      );
  ...
  Future<void> _createPackage() async {
    ...
    if (_useImageUrl) {
      ...
      if (imageToUpload == null) {
        imageToUpload = await AssetToFileHelper.assetToFile(
          'assets/default_thumbnail.webp',
          fileName: 'default_thumbnail_package.webp',
        );
  ...
  Future<void> _showEditDialog(Map<String, dynamic> package) async {
    ...
    final editDescriptionController = TextEditingController(text: package['description'] ?? '');
                ...
                children: [
                  ...
                  _isLoading
                    ...
                    return ListTile(
                                  ...
                                  subtitle: Text(
                                    'Price: ${package['price'] ?? 'N/A'} | Duration: ${package['duration'] ?? 'N/A'} | Classes: ${package['numberOfClasses'] ?? 'N/A'}',
                                  ),
                      trailing: Row(
                        mainAxisSize: MainAxisSize.min,
                        children: [
                          ...
                          IconButton(
                                        ...
                                        onPressed: () => _delete(package['_id'] ?? package['id'] ?? ''),
                          ),
                        ],
                      ),
                );
...
class _FitnessMembershipCardManagerState
  ...
  Future<void> _loadDefaultMedia() async {
    ...
    if (_selectedMedia == null) {
      final file = await AssetToFileHelper.assetToFile(
        'assets/default_thumbnail.webp',
        fileName: 'default_thumbnail_fitness.webp',
      );
      if (file != null && mounted) {
        setState(() {
          _selectedMedia = file;
      ...
      if (mounted) {
        setState(() {
          ...
          try {
            ...
            // Fitness category not found, user can select manually
            print('Fitness category not found: $e');
          }
        });
      }
    } catch (e) {
  ...
  Future<void> _createSubscription() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Fitness subscription created successfully')),
        );
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Error creating fitness subscription: ${e.toString()}')),
        );
      }
    } finally {
      if (mounted) {
                      ...
                      children: [
                        ...
                        const Expanded(
                          child: Column(
                            ...
                            children: [
                              Text(
                                'Fitness Subscriptions List',
                                style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold),
                              ),
                              SizedBox(height: 4),
                              Text(
                                'View and manage all fitness subscriptions',
                                ...
                                child: Padding(
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      const SizedBox(height: 16),
                                      Text(
                                        'No fitness subscriptions found',
                                        ...
                                        child: subscription['media'] != null
                                            ? ClipRRect(
                                                ...
                                                child: Image.network(
                                                  subscription['media'],
                                                  fit: BoxFit.cover,
                                                  errorBuilder: (context, error, stackTrace) => const Icon(Icons.fitness_center, color: Colors.blue),
                                                ),
                                              )
        ...
        Row(
          children: [
            Container(
              ...
              child: const Icon(Icons.fitness_center, color: Colors.blue, size: 28),
            ),
            const SizedBox(width: 16),
            const Expanded(
              child: Column(
                ...
                children: [
                  Text(
                    'Create Fitness Subscription',
                    style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold),
                  ),
                  SizedBox(height: 4),
                  Text(
                    'Add a new fitness course or class',
                    style: TextStyle(fontSize: 14, color: Colors.grey),
                  ),
                ],
              ),
      ...
      child: TextField(
        ...
        SizedBox(
          ...
          child: ElevatedButton.icon(
            ...
            label: Text(
              _isLoading ? 'Creating...' : 'Create Fitness Subscription',
